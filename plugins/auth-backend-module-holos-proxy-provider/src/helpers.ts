    ...
    for (const key of jwks) {
      try {
        ...
        if (!`${error}`.includes('signature verification failed')) {
          throw new AuthenticationError(`could not validate id token: ${error}`)
        }
      }
    }
    throw new AuthenticationError('could not validate id token: signature verification failed')
  };
}
...
async function loadJwksUri(logger: LoggerService, jwksUri: string): Promise<KeyLike[]> {
  try {
